	...
	public class SqlProfile : Profile
	{
		public SqlProfile()
		{
			CreateMap<SqlDataReader, Torneo>()
				...
				.ForMember( dest => dest.BolsaNumber, opt => opt.MapFrom( src => src.GetFieldValue<int>( src.GetOrdinal( "bolsa" ) ) ) )
				...
				.ForMember( dest => dest.GanadorId, opt => opt.MapFrom( src => (Guid)src["ganadorId"] ) )
				.ForMember( dest => dest.Ganador, opt => opt.MapFrom( src => src.GetFieldValue<string>( src.GetOrdinal( "ganador" ) ) ) )
				.IgnoreNoMap();

			CreateMap<SqlDataReader, Posicion>()
				.ForMember( dest => dest.TorneoId, opt => opt.MapFrom( src => (Guid)src["torneo_id"] ) )
				...
				.ForMember( dest => dest.Podio, opt => opt.MapFrom( src => src.GetFieldValue<bool>( src.GetOrdinal( "podio" ) ) ) )
				.ForMember( dest => dest.PremioNumber, opt => opt.MapFrom( src => src.GetFieldValue<decimal>( src.GetOrdinal( "premio" ) ) ) )
				.ForMember( dest => dest.Burbuja, opt => opt.MapFrom( src => src.GetFieldValue<bool>( src.GetOrdinal( "burbuja" ) ) ) )
